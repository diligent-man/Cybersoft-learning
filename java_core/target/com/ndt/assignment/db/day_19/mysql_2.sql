...
BEGIN;
INSERT INTO Books (BookID, Title, Author, Genre, YearPublished)
VALUES (1, 'Dế Mèn Phiêu Lưu Ký', 'Tô Hoài', 'Thiếu nhi', 1941),
...
INSERT INTO Members (MemberID, Name, Gender, City, JoinDate)
VALUES (1, 'Nguyễn Văn A', 'Nam', 'Hanoi', '2020-01-01'),
...
VALUES (1, 1, 1, '2022-03-01', '2022-03-10'),
...
SELECT Borrowings.MemberID, COUNT(Borrowings.MemberID)
FROM Borrowings
GROUP BY Borrowings.MemberID;

# 6.Tìm những sách chưa từng được mượn
SELECT Borrowings.MemberID, COUNT(Borrowings.MemberID) AS borrow_count
FROM Borrowings
GROUP BY Borrowings.MemberID
...
WITH tmp AS (SELECT MemberID, (ReturnDate - BorrowDate) AS thoi_gian_giu_sach
             FROM Borrowings
             ORDER BY thoi_gian_giu_sach DESC
             LIMIT 1)
SELECT Borrowings.MemberID, ReturnDate - BorrowDate
FROM Borrowings
         INNER JOIN tmp ON Borrowings.MemberID = tmp.MemberID
WHERE ReturnDate - BorrowDate = tmp.thoi_gian_giu_sach;

# 9.Liệt kê tên thành viên và số cuốn sách họ đã mượn kèm số sách chưa trả
SELECT b.MemberID,
       COUNT(*)                                              AS `Số sách đã mượn`,
       SUM(CASE WHEN b.ReturnDate IS NULL THEN 1 ELSE 0 END) AS `Số sách chưa trả`
FROM Members m
         JOIN Borrowings b on m.MemberID = b.MemberID
GROUP BY b.MemberID;

# 10.Liệt kê các thành viên đã mượn sách thể loại “Tiểu thuyết” ít nhất 2 lần
SELECT m.Name, COUNT(b2.Genre)
FROM Members m
         JOIN Borrowings b on m.MemberID = b.MemberID
         JOIN Books b2 on b.BookID = b2.BookID
WHERE b2.Genre = 'Tiểu thuyết'
GROUP BY b.MemberID, b2.Genre
HAVING COUNT(b2.Genre) >= 2;

# 11.Tìm thành viên đã mượn sách nhưng chưa trả.
SELECT Borrowings.MemberID
FROM Borrowings
WHERE ReturnDate IS NULL;

# 12.Liệt kê 3 thể loại sách được mượn nhiều nhất
SELECT Borrowings.BookID, COUNT(Borrowings.BookID)
FROM Borrowings
GROUP BY Borrowings.BookID
ORDER BY COUNT(Borrowings.BookID) DESC
LIMIT 3;

# 13.Liệt kê tên thành viên và tên sách họ đã mượn lần gần nhất
WITH tmp AS (SELECT m.Name,
                    b2.Title,
                    ROW_NUMBER() OVER (PARTITION BY b.MemberID ORDER BY b.BorrowDate DESC) as rn
             FROM Members m
                      JOIN Borrowings b
                           on m.MemberID = b.MemberID
                      JOIN Books b2 on b.BookID = b2.BookID)
SELECT tmp.Name, tmp.Title
FROM tmp
WHERE rn = 1;
