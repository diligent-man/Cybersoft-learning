...
SELECT *,
       SUM(COUNT(task_id)) OVER ()                                       AS total_task,
       SUM(COUNT(task_id)) OVER (PARTITION BY status_name)      AS 'total_task_by_status',
...
FROM UserInProject
WHERE task_count > 0
GROUP BY project_id, task_id, task_name, user_id, fullname, status_name, email, status_color, start_date, end_date
ORDER BY user_id;
;
