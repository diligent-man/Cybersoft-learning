...
@WebFilter(
    filterName = "roleTableFilter",
    urlPatterns = {"/role", "/role-add"}
)
    ...
    protected void doFilter(HttpServletRequest req, HttpServletResponse res, FilterChain chain) throws IOException, ServletException {
        ...
        */
        ...
        if (role != null) {
            ...
            } else if (path.equals("/role") && ALLOWED_VIEW_ROLES.contains(role)) {
                chain.doFilter(req, res);
            } else if (path.equals("/role-add") && ALLOWED_MOD_ROLES.contains(role)) {
                chain.doFilter(req, res);
            } else {
